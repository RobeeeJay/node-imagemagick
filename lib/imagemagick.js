...
function parseIdentify(stdout) {
  var lines = stdout.split(/\n/);
  
  // remove top line with filename
  lines.shift();
  
  return parseSection({ lines: lines });
}

function parseSection(linesobj) {
  var properties = {};
  var indent = linesobj.lines[0].search(/\S/);

  while (linesobj.lines.length) {
        if (linesobj.lines[0].search(/\S/) < indent)
          return properties;

        var keyvalue = linesobj.lines.shift().replace(/^\s+/, "").split(/:/);
        
        if (keyvalue.length > 2)
          keyvalue = [ keyvalue.shift(), keyvalue.join(":") ];
        
        if (keyvalue.length == 2) {
          if (keyvalue[1].length)
                properties[keyvalue[0].toLowerCase()] = keyvalue[1].replace(/^\s+/, "");
          else
                properties[keyvalue[0].toLowerCase()] = parseSection(linesobj);
        }
  }
  ...
  var proc = exec2(exports.identify.path, args, {timeout:120000}, function(err, stdout, stderr) {
    ...
    if (!err) {
      if (isCustom) {
        ...
        {
          result = { width: 0, height: 0, format: "unknown", depth: 0 };
        }
      }
    }
    callback(err, result);
